...
def estimate_weights_enkf_augmented(model, w0, xs, ys, x_test, y_test, dx, meas_model, timesteps, num_epochs, ensemble_size, batch_size, r, logger, adapt_r="None", loss_function="mse", initial_noise=0.03, Ts=None, save_interval=100, parallelize=False):
    ...
    for epoch in range(num_epochs):
        for i in range(timesteps):
            ...
            if use_ENKF: ENKFUpdate = np.mean(X5, axis=1)[:, None]

            print("SGDUpdateMax", np.max(SGDUpdate))
            if use_ENKF: print("ENKFUpdateMax", np.max(ENKFUpdate))
            print("SGDUpdateNorm", np.linalg.norm(SGDUpdate))
            if use_ENKF: print("ENKFUpdateNorm", np.linalg.norm(ENKFUpdate))

            print(SGDUpdate.shape)#, ENKFUpdate.shape)

            if use_ENKF: print("Cosine", np.matmul(SGDUpdate.T, ENKFUpdate) / (np.linalg.norm(SGDUpdate) * np.linalg.norm(ENKFUpdate)))
